...
class _CollectionItemState extends State<CollectionItem> {
  ...
  Widget build(BuildContext context) {
    return Consumer<SecretKeyProvider>(
      builder: (context, secretKeyProvider, _) => InkWell(
        onTap: (){
          setState(() {
        ...
        onSecondaryTapUp: (details){
          if(secretKeyProvider.value.isEmpty){
            return;
          }
          setState(() {
            focusNode.requestFocus();
            ...
            children: [
              if ([ItemMode.VIEW, ItemMode.MENU].contains(mode)) TextButton(
                  ...
                  child: Container(
                    ...
                    child:  secretKeyProvider.value.isEmpty? Text(widget.collection.name, style: EncryptedTxtStyle.classic(12, ColorPalette.getWhite(1)),) : Text(EncryptionService.decode(widget.collection.name, secretKeyProvider.value)),
                  )
              ) else const SizedBox(),
                ...
                child: TextField(
                  ...
                  onSubmitted: (value) async {
                    if(value.isNotEmpty && value.length > 3 && value.length <= 50){
                      try{
                        await DbService.updateCollection(widget.collection.id, (name: value), secretKeyProvider.value);
                      }
                      catch(e){
                        textEditingController.text = widget.collection.name;
                        print(e);
